...
class U_Net():
    ...
    def build_unet(self, n_filters=16, dropout=0.1, batchnorm=True, padding='same'):
        ...
        # 定义一个多次使用的卷积块
        def conv2d_block(input_tensor, n_filters=16, kernel_size=2, batchnorm=True, padding='same'):
            # the first layer
        ...
        # contracting path
        c1 = conv2d_block(img, n_filters=n_filters * 4, kernel_size=2, batchnorm=batchnorm, padding=padding)
        p1 = MaxPooling2D((2, 2))(c1)
        p1 = Dropout(dropout * 0.5)(p1)

        c2 = conv2d_block(p1, n_filters=n_filters * 8, kernel_size=2, batchnorm=batchnorm, padding=padding)
        p2 = MaxPooling2D((2, 2))(c2)
        p2 = Dropout(dropout)(p2)

        c3 = conv2d_block(p2, n_filters=n_filters * 16, kernel_size=2, batchnorm=batchnorm, padding=padding)
        p3 = MaxPooling2D((2, 2))(c3)
        p3 = Dropout(dropout)(p3)

        c4 = conv2d_block(p3, n_filters=n_filters * 32, kernel_size=2, batchnorm=batchnorm, padding=padding)
        p4 = MaxPooling2D((2, 2))(c4)
        p4 = Dropout(dropout)(p4)

        # 中间的一个特征层
        c5 = conv2d_block(p4, n_filters=n_filters * 64, kernel_size=2, batchnorm=batchnorm, padding=padding)

        # extending path
        u6 = Conv2DTranspose(n_filters * 32, (2, 2), strides=(2, 2), padding='same')(c5)
        u6 = concatenate([u6, c4])
        u6 = Dropout(dropout)(u6)
        c6 = conv2d_block(u6, n_filters=n_filters * 32, kernel_size=2, batchnorm=batchnorm, padding=padding)

        u7 = Conv2DTranspose(n_filters * 16, (2, 2), strides=(2, 2), padding='same')(c6)
        u7 = concatenate([u7, c3])
        u7 = Dropout(dropout)(u7)
        c7 = conv2d_block(u7, n_filters=n_filters * 16, kernel_size=2, batchnorm=batchnorm, padding=padding)

        u8 = Conv2DTranspose(n_filters * 8, (2, 2), strides=(2, 2), padding='same')(c7)
        u8 = concatenate([u8, c2])
        u8 = Dropout(dropout)(u8)
        c8 = conv2d_block(u8, n_filters=n_filters * 8, kernel_size=2, batchnorm=batchnorm, padding=padding)

        u9 = Conv2DTranspose(n_filters * 4, (2, 2), strides=(2, 2), padding='same')(c8)
        u9 = concatenate([u9, c1])
        u9 = Dropout(dropout)(u9)
        c9 = conv2d_block(u9, n_filters=n_filters * 4, kernel_size=3, batchnorm=batchnorm, padding=padding)
        ...
        for i in range(256):
            for j in range(256):
                if y_true[i][j] == y_pred[i][j]:
                    count = count + 1
        return count / (256 * 256)

    def load_data(self):
        x_train = []  # 定义一个空列表，用于保存数据集
        x_label = []
        for file in glob(r"C:\Users\86156\Desktop\train"):  # 获取文件夹名称
            for filename in glob(file + '/*'):  # 获取文件夹中的文件
                img = np.array(Image.open(filename), dtype='float32') / 255
                x_train.append(img[:, :])
        for file in glob(r"C:\Users\86156\Desktop\train_label"):
        ...
        # 图片有150左右，按9:1进行分配
        return x_train[:135, :, :], x_label[:135, :, :], x_train[135:, :, :], x_label[135:, :, :]

    def train(self, epochs=20, batch_size=32):
        os.makedirs(r"C:\Users\86156\Desktop\weights", exist_ok=True)
        # 获得数据
        ...
        callbacks = [EarlyStopping(patience=100, verbose=2),
                     ...
                     ModelCheckpoint(r"C:\Users\86156\Desktop\weights\best_model.h5", verbose=2, save_best_only=True)]
        ...
        fig.savefig(r"C:\Users\86156\Desktop\tupian/curve.png", bbox_inches='tight', pad_inches=0.1)  # 保存绘制曲线的图片
        plt.close()

    def test(self, batch_size=1):
        os.makedirs(r"C:\Users\86156\Desktop\evaluation\test_results", exist_ok=True)
        self.unet.load_weights(r"C:\Users\86156\Desktop\weights\best_model.h5")
        # 获得数据
        x_train, x_label, y_train, y_label = self.load_data()
        test_num = y_train.shape[0]
        index, step = 0, 0
        self.unet.evaluate(x_train, x_label)
        n = 0.0
        while index < test_num:
            ...
            mask.save(r"C:\Users\86156\Desktop\evaluation\test_results/" + str(step) + '.png')
            mask_true = Image.fromarray(np.uint8(mask_true * 255))
            mask_true.save(r"C:\Users\86156\Desktop\evaluation\test_results/" + str(step) + 'true.png')
            index += batch_size
        # acc = n / test_num * 100
